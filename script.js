...
//リサイズ処理
  ...
  function loop(){
    ...
    if(frameId % 8 == 0) {
        ...
        new Dot(canvas.width*Math.random()+canvas.width-canvas.width*4*Math.random(), -canvas.height/2, getRandom(1, 3),  getRandom(2, 4),"#90BE6D");
        new Dot(canvas.width*Math.random()+canvas.width-canvas.width*4*Math.random(), -canvas.height/2, getRandom(1, 3),  getRandom(2, 4),"#577590");
    }
